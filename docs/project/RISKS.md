...
| R-9 | GOLF-108/109 (map course-pin visibility) hinge on a product decision — "what course pins show in which trip-builder tab" — not just a code fix | Under-specified work → coding agent guesses → rework, or scope creep | Active | `GOLF-107-uxpass.md` states the requirement + the one open question (nearby-in-Itinerary: always/zoom/toggle). Owner to confirm before GOLF-108 goes to a coding agent. |
