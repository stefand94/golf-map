...
| R-8 | Basemap uses OSM's public tile server (`{s}.tile.openstreetmap.org`), which has a usage policy that excludes production/high-volume apps and can rate-limit or block | Map tiles fail to load for real visitors after go-live | Low now, rising with traffic | GOLF-105 (rescoped, DEC-010): move to Esri keyless ArcGIS Online tiles + a street/satellite toggle. Removes the OSM production-ban; residual = Esri keyless endpoints are a usage policy not a contract (far more permissive; Esri could change them) → fully-contracted or vector basemap is GOLF-106 if ever needed. Not urgent at demo traffic. |
