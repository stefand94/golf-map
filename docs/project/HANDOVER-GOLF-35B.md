...
5. **The Worker moves to `api.golftripper.uk`,** and `ORS_PROXY_URL` in `js/ors.js` points there. **Once that's verified, the `workers.dev` address is switched off** (BA 2026-09-23); otherwise scripts could bypass the rate limit by calling it directly. This is the final step, after the burst test. **Amended 2026-09-23 (dev finding):** a git deploy re-enables workers.dev unless the config says otherwise, so this is done with a commit that sets `workers_dev` and `preview_urls` to `false` in `wrangler.jsonc`, not a dashboard click. It also closes the per-version preview URLs, a second way around the rate limit. Verified by curling workers.dev.

**Push order (BA 2026-09-23):** two pushes, not one. Push 1 ships the Worker only (allowlist, `Max-Age`, custom domain). Push 2 ships the site (`js/ors.js`, `_middleware.js`, docs) once `X-Worker-Build` on api.golftripper.uk shows the new build. This keeps the live site from pointing at a Worker that doesn't yet accept golftripper.uk.
