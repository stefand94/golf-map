...
| `coordSrc` | string, optional | Where this record's coordinate came from, when it is not the original import. Only value in use: `"osm"` (GOLF-161 / DEC-023 — re-sourced from OpenStreetMap for England and Scotland, whose coordinates originally came from governing-body directories whose terms bear on republishing). Absent means the original source, which is fine and not a defect. |
| `osm` | string, optional | The OpenStreetMap object the coordinate came from, e.g. `"node/783442816"` — present exactly when `coordSrc:"osm"`. This is the ODbL attribution trail: it says *which* object, so a wrong pin can be checked against OSM rather than argued about. The user-facing credit is on the map itself (`js/map.js`, added to the Leaflet attribution control). Written by `scripts/merge_osm_coords.py`. |
...
| `wd`, `we` | string | Weekday / weekend green fee, as free text (not always a single number — ranges, "Members only", "Ask club" etc. are common and intentional). Kept on every course even once `fee` (below) exists — `fee` is additive, not a replacement. |
| `fee` | object, optional | **GOLF-97 v1 — LEGACY / FROZEN.** No new `fee` objects are written; existing ones (England Top 100 only) stay as the middle tier of the fee-resolution chain until re-researched to `feeV2`. Shape: `{weekday:{min,max}, weekend:{min,max}, weekendTwilight?:{min,max}, confidence, lastVerified}`. A single published figure sets `min===max`. `confidence` is one of `'published-range'`, `'published-from-only'`, `'estimated'`, `'poa'` (`min`/`max` both `null`). Read via the same helpers as `feeV2` (below); never `extractFee()` directly on a `fee`-bearing course. |
| `feeV2` | object, optional | **GOLF-120 — the current green-fee schema.** Granular, flexible, published-card-verbatim where possible. Shape: `{currency:'GBP'|'EUR'|'ZAR'|'USD', source, lastVerified, confidence, notes?, cart?, seasons:[…]}`. `confidence` ∈ `'published-rates'` (club rate card copied) / `'published-from-only'` (club only quotes "from £X") / `'estimated'` (no card found — aggregator/press, UI-badged) / `'poa'` (no visitor rate bookable — `seasons` **must** be `[]`). Each `seasons[]` entry is `{name, months?:number[1-12], rates:[…]}`; each `rates[]` entry is `{day:'weekday'|'weekend'|'friday'|'any', amount, amountMax?, isFrom?, timeBand?:'anytime'|'morning'|'afternoon'|'twilight'|'super-twilight', bandStart?:'HH:MM', holes?:18|9|'day', playerType?, cartIncluded?}`. Optional `cart` side-channel: `{status:'included'|'mandatory'|'extra', amount?, per?:'cart'|'person'|…}` — a `'mandatory'` cart becomes its **own** "Compulsory buggy" Costs-tab line (owner decision Q6), never folded into the green fee. Read via `feeRangeFor()` / `feeNumberFor()` / `feeNumberForDate()` / `feeRangeForDate()` / `feeV2Label()` (`js/trip-geo.js`), which apply the §3 "highest applicable" derivation (rule A no-date → max over all in-scope seasons; rule B by date → in-season + Sat/Sun-only weekend, **no** bank-holiday list; rule C by date+time → latest `bandStart` ≤ tee time). Display headline: `£X` (lone rate) / `£X–£Y` (published band) / `Up to £X` (derived in-season ceiling) / `from £X` (club "from" price) / `POA`. `feeV2` is tier 1, the v1 `fee` object tier 2, the `wd`/`we` regex tier 3 — a course with no `feeV2` renders exactly as before. Full spec: `docs/project/GOLF-120-schema-v2-proposal.md`. |
| `conf` | string | Data confidence: `"club"` (from the club's own site), `"press"` (published guide/trade press), or `"est"` (indicative/unverified). Drives an on-card confidence badge. GOLF-25/26/77/78: all `C_SCOTLAND`/`C_WALES`/`C_IRELAND`/`C_SOUTHAFRICA` fee/access/architect/note fields are marked `"est"` — only coordinates/phone/website came from a live-verified source (the DotGolf API) this round. |
...
### London-catchment-only fields (present on `C` entries, absent on `C_TOP100`/`C_SCOTLAND`/`C_WALES`/`C_IRELAND`/`C_SOUTHAFRICA`)
...
| `topSouthAfrica:1` | boolean flag | GOLF-78 / GOLF-121a: nation gate for South Africa — present (and always `1`) on every `C_SOUTHAFRICA` entry, curated and bulk alike. |
| `zaRanked:1` | boolean flag | GOLF-121d: marks a South African course that appears on a recognised top-100 list (union of satop100courses.com + Top100GolfCourses.com). ~107 of the ~421 `C_SOUTHAFRICA` entries carry it. **The SA map/pins are ringfenced to these** — `courseShownOnMap()` in `js/explore.js` hides `topSouthAfrica:1` entries that lack `zaRanked:1`; they stay in the data file (recoverable, ready for a future "show all SA courses" toggle) but do not surface on the map. Always paired with a `t100.za` number. |
