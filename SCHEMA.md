...
| `C_SOUTHAFRICA` | array of course objects | GOLF-78: 19 curated notable South African courses (`data/courses-southafrica.js`). Appended onto `C` via `C.push(...C_SOUTHAFRICA)`, same convention as `C_TOP100`/`C_IRELAND`. Fees in ZAR (`R`) — a new currency for this map. |
...
| `topSouthAfrica:1` | boolean flag | GOLF-78: marks an entry as one of the 19 curated South Africa courses. Only ever present (and always `1`) on `C_SOUTHAFRICA` entries. |
