...
| `fee` | object, optional | GOLF-97: a structured, banded green-fee shape, hand/web-researched (GOLF-98) — the fix for `wd`/`we`'s free-text/regex-averaged figures silently understating real peak pricing. Shape: `{weekday:{min,max}, weekend:{min,max}, weekendTwilight?:{min,max}, confidence, lastVerified}`. A single published figure sets `min===max`. `confidence` is one of `'published-range'` (the club states a real min–max), `'published-from-only'` (only a "from £X" figure exists — `max` is a researched estimate, not the club's own number), `'estimated'` (no published figure — a placeholder, mirrors `conf:'est'`), or `'poa'` (members-only/ask-club — `min`/`max` both `null`). `lastVerified` is an ISO date. Read via `feeRangeFor()`/`feeNumberFor()`/`feeNumberForDate()` (`js/trip-geo.js`) — never `extractFee()` directly on a `fee`-bearing course; those helpers fall back to the legacy `wd`/`we` regex parse for any course without a `fee` object, so this is non-breaking. |
