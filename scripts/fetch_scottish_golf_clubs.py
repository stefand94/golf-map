#!/usr/bin/env python3
"""
GOLF-25: look up golf clubs against Scottish Golf's public "Find a
facility" club-finder API. Same undocumented-but-public, unauthenticated
API shape as England Golf's (both run on the "DotGolf" white-label
platform, confirmed live via the Browser tool by inspecting network
traffic on scottishgolf.org — same endpoint names, same response fields).
This is a straight copy of fetch_england_golf_clubs.py with BASE swapped;
see that script's docstring for endpoint details this mirrors exactly.

Input: a JSON file mapping a stable key -> the club name to search for, e.g.:
    {"st-andrews-old": "St Andrews Links (Old Course)", ...}

Output: writes scripts/output/scottish_golf_clubs.json — same shape as
england_golf_clubs.json, so merge_club_details.py / merge_club_images.py
work against it unchanged.

Usage:
    python3 scripts/fetch_scottish_golf_clubs.py --names-file names.json \\
        [--out scripts/output/scottish_golf_clubs.json]
...
BASE = "https://www.scottishgolf.org/api/clubs"
...
def main():
    ...
    parser.add_argument("--out", default="scripts/output/scottish_golf_clubs.json")
    parser.add_argument(
    ...
    out = {
        ...
        "source": f"{BASE} (Scottish Golf Find a Facility, undocumented public API)",
        "requested": len(names),
