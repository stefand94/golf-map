#!/usr/bin/env python3
"""
GOLF-26: look up golf clubs against Wales Golf's public "Find a
...
traffic on walesgolf.org — same endpoint names, same response fields).
...
Input: a JSON file mapping a stable key -> the club name to search for, e.g.:
    {"royal-porthcawl": "Royal Porthcawl Golf Club", ...}

Output: writes scripts/output/wales_golf_clubs.json — same shape as
england_golf_clubs.json, so merge_club_details.py / merge_club_images.py
work against it unchanged.

Usage:
    python3 scripts/fetch_wales_golf_clubs.py --names-file names.json \\
        [--out scripts/output/wales_golf_clubs.json]
...
BASE = "https://www.walesgolf.org/api/clubs"
...
def main():
    ...
    parser.add_argument("--out", default="scripts/output/wales_golf_clubs.json")
    parser.add_argument(
    ...
    out = {
        ...
        "source": f"{BASE} (Wales Golf Find a Facility, undocumented public API)",
        "requested": len(names),
