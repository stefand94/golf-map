...
function itinLegRowHTML(l){
  ...
  return`<div class="tb-day-course tb-item-${l.type}" style="cursor:default">
    ...
    <div class="tb-item-main"><span class="tb-item-name">${esc(l.name)}</span></div>
...
function tbItinHotelRailHTML(){
  ...
  return`<div class="itin-rail">${rows.map(r=>`<div class="itin-rail-row"><div class="itin-rail-dot"></div>
      ...
      <div class="itin-flat-row"><span class="itin-hotel-name-md">🏨 ${esc(r.name)}</span><span class="itin-hotel-price-md">${tbMoney(r.price)}</span></div>
    </div>`).join('')}</div>`;
}
...
function tripCostLineItems(){
  ...
  tripDays.forEach((d,idx)=>tripDayItems(d).forEach(it=>
    items.push({label:tripItemName(it),cat:CAT[it.type]||'Stop',amount:tripItemPrice(d,it),day:idx+1})));
